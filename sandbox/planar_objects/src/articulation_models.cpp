...
void ManifoldModel::getError(double &err_trans,double &err_rot) {
  err_rot = 0;
  err_trans = 0;
  if(track->obs_history.size() == 0) return;

  double sum_trans=0,sum_rot = 0;
  for(size_t j=0;j<track->obs_history.size();j++) {
    btTransform error = track->obs_history[j].tf.inverseTimes(getPrediction( getConfiguration(track->obs_history[j].tf) ));
...
std::vector< std::pair<double,double> > ManifoldModel::getRange() {
  ...
  if(track->obs_history.size()==0) return range;
