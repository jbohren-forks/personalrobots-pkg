...
"""
 usage: %(progname)s [<image_file> | --folder=<folder_name>] [--margin=<margin>] [--aspect_ratio=<ratio>]
 ...
 The XML files should be in <folder_name>/annotations/ and the corresponding
 image files should be in <folder_name>/images/ with the same name except
 for the extension.
...
def main(argv, stdout, environ):
  ...
  if folder:
    annotations_input_folder = os.path.join(folder,"annotations")
    polygons_output_folder = os.path.join(folder,"polygons")
    if not os.path.exists(polygons_output_folder):
      os.mkdir(polygons_output_folder)
    xml_filenames = os.listdir(annotations_input_folder)
  elif (len(args) > 0):
    annotations_input_folder = ""
    polygons_output_folder = ""
    xml_filenames = args
  else:
    usage(progname)
    return
  ...
  for xml_input_filename in xml_filenames:
    if (len(xml_input_filename) >= 4) and (xml_input_filename[-4:] == ".xml"):
      full_xml_input_filename = os.path.join(annotations_input_folder, xml_input_filename)
      xml_string = oriented_bounding_boxes.convert_polygons_to_absolute_coord(full_xml_input_filename)
      xml_output_filename = xml_input_filename[0:-4] + "_polygons.xml"
      xml_output_file = open(os.path.join(polygons_output_folder, xml_output_filename), 'w')
