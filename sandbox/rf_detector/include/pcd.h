...
inline void Sample_interest_pointNoTF( sensor_msgs::PointCloud& pc, 
    Vector< geometry_msgs::Point32>& interest_pts, 
    vector< cv::Point>& image_patch_centers,
    sensor_msgs::CameraInfo& cinfo,
    cv::Size SampleSteps, cv::Size ImgSize){
    
    // transform from pc to image pixel
    Vector<geometry_msgs::Point> pp;
    cout << "start GetPointCloud2pointNoTF" <<endl;
    GetPointCloud2pointNoTF( pc, pp, cinfo);
    cout << "finish GetPointCloud2pointNoTF" <<endl;
    ...
    CvMat * valid_pp;
    unsigned int chandim = 2;
    unsigned int neighbor_grid = 20;
    valid_pp = cvCreateMat( gridSize.height, gridSize.width, CV_16UC2);
	// three chan: interest_pts_ind, image_x, image_y
    cvZero(valid_pp);
    unsigned short int* uchar_ptr;
    cout << "start valid_pp building" <<endl;
    unsigned int gridx_ori;
    unsigned int gridy_ori;
    for (unsigned int q = 0; q < pp.size(); q++){
	gridx_ori = pp[q].x/SampleSteps.width;
	gridy_ori = pp[q].y/SampleSteps.height;
	for (unsigned int gridx = std::max((int)(gridx_ori-neighbor_grid),0);		 
		gridx < std::min( (int)(gridx_ori+neighbor_grid),gridSize.width); gridx++){
		for (unsigned int gridy = std::max((int)(gridy_ori-neighbor_grid),0); 
			gridy < std::min((int)(gridy_ori+neighbor_grid),gridSize.height); gridy++){
			uchar_ptr = (unsigned short int*) (valid_pp->data.ptr + gridy*valid_pp->step);
			if (uchar_ptr[ gridx*chandim] !=0){
				uchar_ptr[ gridx*chandim] = q;
				uchar_ptr[ gridx*chandim+1] = dist2d( gridx-pp[q].x, gridy-pp[q].y);
			}else if( dist2d( pp[q].x-gridx, pp[q].y-gridy)
				  < uchar_ptr[ gridx*chandim+1] ){
				uchar_ptr[ gridx*chandim] = q;
				uchar_ptr[ gridx*chandim+1] = dist2d( gridx-pp[q].x, gridy-pp[q].y);	
			}
			//if (q == 0){
			//	cout << "gridx_ori" << gridx_ori << "gridx" << gridx <<
			//		"gridy_ori" << gridy_ori << "gridy" << gridy << endl;
			//}
		}
	}
    }
    cout << "finish valid_pp building" <<endl;

    // Sample interest_pts
    geometry_msgs::Point32 point32_tmp;
    cout << "start Sample interest_pts" <<endl;
    for (unsigned int y=0, gridy=0; y< ImgSize.height; gridy++, y+=SampleSteps.height){
        ...
        for (unsigned int x=0, gridx=0; x< ImgSize.width; gridx++, x+=SampleSteps.width){
            if ( uchar_ptr[chandim*gridx] != 0){
                point32_tmp.x = pc.points[ uchar_ptr[chandim*gridx]].x;
                point32_tmp.y = pc.points[ uchar_ptr[chandim*gridx]].y;
                point32_tmp.z = pc.points[ uchar_ptr[chandim*gridx]].z;
    ...
    cout << "finish Sample interest_pts" <<endl;
}
