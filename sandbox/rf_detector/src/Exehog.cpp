...
//            cout << i << " "<< fbr[i].cols <<  " " <<fbr[i].rows<<endl;
        }
//        cout << "ymap useful" << endl;
...
//                cout << x<<" " <<i<<" "<<fbrptr[i][xmap[x]];
            }
//            cout << "--" << gradPtr[x*2] << " " << gradPtr[x*2+1] << endl;
        }
//        cout << "xmap useful" << endl;
    }
        ...
        bool _useCache, Size _cacheStride, const vector<Mat>& fbr, bool PbFlag)
{
    ...
    if (PbFlag){
        ...
        //cout << "img" << _img.cols<<" " << _img.rows << endl;
        descriptor->computeGradient(_img, grad, qangle, _paddingTL, _paddingBR);
    }
    //cout << "grad" << grad.cols<<" " << grad.rows << endl;
    //cout << "qangle" << qangle.cols<<" " << qangle.rows << endl;
...
const float* HOGCache::getBlock(Point pt, float* buf)
{
    ...
    if( useCache )
    {
        ...
        if( computedFlag != 0 )
        {
//            std::cout << "get from cache, saving" << std::endl;
            return blockHist;}
...
//        paddingTL.width = 0;
//        paddingTL.height = 0;
//        paddingTL.width = (int)alignSize( paddingTL.width, cacheStride.width);
//        paddingTL.height = (int)alignSize( paddingTL.height, cacheStride.height);
//        paddingBR.width = 0;
//        paddingBR.height = 0;
//        paddingBR.width = (int)alignSize( paddingBR.width, cacheStride.width);
//        paddingBR.height = (int)alignSize( paddingBR.height, cacheStride.height);
//    }
...
//    HOGCache cache(this, img, paddingTL, paddingBR, nwindows == 0, cacheStride);
//    HOGCache cache(this, img, paddingTL, paddingBR, false, cacheStride);
    ...
    {
//        cout << "win count" << i << endl;
        float* descriptor = &descriptors[i*dsize];

        Point pt0;
        if( !locationsOriEmptyFlag)
        {
//            std::cout << "locationsOriEmptyFlag" << std::endl;
            pt0 = locations[i];
            if( pt0.x < -paddingTL.width || pt0.x > img.cols + paddingBR.width - winSize.width ||
                ...
                //cout << "location out of range" << pt0.x<<" " << pt0.y << " " << winSize.width <<" " << winSize.height<<
        ...
        else
        {
            ...
            //std::cout << cacheStride.width << " " <<cacheStride.height << " " << pt0.x << " " << pt0.y << std::endl;
...
//                sumup += dst[k]*dst[k];
//
//            sumup = std::sqrt(sumup);
//            std::cout << "sumup="<< sumup << std::endl;
        }
...
{
    hits.clear();
//    hitsIds.clear(); // -TODO multithread
//    hitsConfs.clear();
...
//        padding.width = 0;
//        padding.height = 0;
//        padding.width = (int)alignSize( padding.width, cacheStride.width);
//        padding.height = (int)alignSize( padding.height, cacheStride.height);
//    }
...
//        cout << vecTestFeature.at(0).at(0) << " " << vecTestFeature.at(vecTestFeature.size()-1).at(0) << " "
//            << vecTestFeature.size()<< endl;
//            cout<< "using rfpredict " << endl;
        rf.RFPredict(vecTestFeature, vecTestLeafNo);
//            cout<< "finished using rfpredict " << endl;
...
//            cout << "FgBgProb_inst " << FgBgProb_inst << endl;
...
//        cout << endl;
//        cout << hits.size() << " " << hitsIds.size() << " " << hitsConfs.size() << endl;
    }
}
...
//        std::cout << "scale" << i << std::endl;
        ...
        if (PbFlag){
            ...
            if( sz == img.size() ){
                for (int bid=0; bid<smallerfbr.size();bid++)
...
//            winSize.width = scaledWinSize[i].width;
//            winSize.height = scaledWinSize[i].height;
            scale = (double)(scaledWinSize[i].width)/(double) (winSize.width);
//            cout << "scale" << scale << "scaleedWin " <<scaledWinSize[i].width<<" winSize"<<winSize.width <<endl; //assume Win is square
            Size sz(cvRound(img.cols/scale), cvRound(img.rows/scale));
            Mat smallerImg(sz, img.type(), tdata.smallerImgBuf.data);
...
//            cout <<"smallerImgSize" << smallerImg.size().width <<" " <<smallerImg.size().height<<endl;
            // modify locations
...
//            cout<< "using rfpredict " << endl;
            ...
            //cout << des_ptr_3d[i]->size() << endl;
            //cout << vecTestFeature.size() <<endl;
            for (unsigned int j =0; j < (vecTestFeature.size()+des_ptr_3d[i]->size()); j++){
                if (j<vecTestFeature.size()){
                    //cout << "vecTestFeature" << vecTestFeature[j][0] <<endl;
                    vecTestFeatureApp[j].push_back( vecTestFeature[j][0]);
                }else{
//                    cout << "des_ptr_3d" << (*(des_ptr_3d[i]))[j-vecTestFeature.size()];
...
float getMedian( float SmallerValue, float BiggerValue, bool SmallFlag){
    if (SmallFlag){
        return SmallerValue;
    }else{
...
void AddWeighted( float* ptr1, float weight1, float* ptr2, float weight2, IplImage* des){
    ...
    vector< Vector<Rect> >& ObjCenterWindowsAll, vector< vector<float> > & ObjCenterConfAll, bool VisualizationFlag)
{
    ...
    float RectOverlapRatio = 0.2;// non-maximum suppression
...
//    Size ImgSize = Img.size();
    ...
    acc_scale_count.push_back(scale_count.at(0));
    for (int i = 1; i<nScale; i++){
        acc_scale_count.push_back( acc_scale_count.at(i-1)+scale_count.at(i));
    }

    vector< vector< CvMat* > > VotesMap;
    vector< vector<vector< float> > > vecASMedianCandSmaller;
    vector< vector<vector< float> > > vecASMedianCandBigger;
    vector< vector<vector< bool> > > vecASMedianCandSmallerFlag;
    vector< vector<vector< float> > > vecLowResVotes;
    vector< vector< int> > SampleCount;
    VotesMap.resize(nScale);
    vecASMedianCandSmaller.resize(nScale);
    vecASMedianCandBigger.resize(nScale);
    vecASMedianCandSmallerFlag.resize(nScale);
    vecLowResVotes.resize(nScale);
    SampleCount.resize(nScale);
    int ImgArea = ImgSize.width*ImgSize.height;
    for (int i = 0; i<nScale; i++){
        VotesMap.at(i).resize(MaxViewId);
        vecASMedianCandSmaller.at(i).resize(MaxViewId);
        vecASMedianCandBigger.at(i).resize(MaxViewId);
        vecASMedianCandSmallerFlag.at(i).resize(MaxViewId);
        vecLowResVotes.at(i).resize(MaxViewId);
        SampleCount.at(i).resize(MaxViewId,0);
    ...
    for (int SampleId = 0; SampleId < nSamples; SampleId++)// loop through samples
    {
        ...
        if (SampleId > acc_scale_count.at(scaleId)){
            scaleId++;
//            cout << scaleId << endl;
        }
        for (int k=0; k < MaxViewId; k++)
...
//                cout << patchId << endl;
            ...
            if (LosResPtr >=LowResMapSizeArea || scaleId >= nScale || viewId >= MaxViewId)
                cout << LosResPtr << endl;
            vecLowResVotes[scaleId][viewId][LosResPtr] += VoteWeights[viewId];
            SampleCount[scaleId][viewId]++;
            ptr =  (float*)( VotesMap[scaleId][viewId]->data.ptr + SubVote.y*VotesMap[scaleId][viewId]->step);
            ptr[SubVote.x] += VoteWeights[viewId];
            CalMedian( AspectRatio, vecASMedianCandSmaller[scaleId][viewId][SubVote.y * ImgSize.width + SubVote.x],
                vecASMedianCandBigger[scaleId][viewId][SubVote.y * ImgSize.width + SubVote.x],
                vecASMedianCandSmallerFlag[scaleId][viewId][SubVote.y * ImgSize.width + SubVote.x]);
        }
    }
    ...
    for ( viewId = 0; viewId < MaxViewId; viewId++){
        ...
        for ( scaleId = scaleIdStart; scaleId < nScale-scaleIdEndMargin; scaleId++){ // the biggest and smallest scale is only used for averaging
            ...
            if (count != 0){
                ...
                sort( LowResInds.begin(), LowResInds.end(), index_cmp<vector<float>&>(vecLowResVotes[scaleId][viewId]) );
                for (int i = 0; i <  (nCandidatePerDim*nCandidatePerDim); i++){
                    ...
                    MeanShiftIter = cvMeanShift( targetVoteMap, window, TermCriteria_, &comp);
...
Vector<float> HogFast::getDefaultPeopleDetector()
{
