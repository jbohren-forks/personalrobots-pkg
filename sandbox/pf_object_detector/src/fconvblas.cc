 ...
 * response of a set of filters with a feature map.  We use threads to
 * compute responses in parallel.  We use blas for computing
 * correlations quickly.  
 *
 * To compile use mex with '-lmwblas' or '-lmwlapack' depending on
...
void *process(void *thread_arg) {
  ...
  for (int x = 0; x < C_dims[1]; x++) {
    for (int y = 0; y < B_dims[0]; y++) {
      ...
      //cblas_dgemv(chn, &m, &n, &one, A_off, &lda, B_off, &incx, &one, C_off, &incy);
      F77_dgemv(chn, &m, &n, &one, A_off, &lda, B_off, &incx, &one, C_off, &incy);
