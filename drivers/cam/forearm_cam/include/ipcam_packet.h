 ...
 *
 * The PacketDiscover is valid in all camera modes. It consists only of a PacketGeneric
 * with the type field set to PKTT_DISCOVER. When received, the camera will generate an
 * PacketAnnounce in reply.
 *
 * @see PacketAnnounce
 */
typedef struct PACKED_ATTRIBUTE { PacketGeneric hdr; } PacketDiscover;
 ...
 * Each page consists of 528 bytes and is requested by an address in the following format:
 *
 *  The address field Holds the 32 bit starting address:
 *                      \li 10 MSB must be zero
 *                      \li next 12 bits are page address
 *                      \li next 10 bits are start offset within the page (normally zero)
 *
 ...
 *
 * The AT45DB161D Atmel Dataflash used in this product has 4096 user-accessible flash pages of 528 bytes each.
 *
 * Valid States:
 ...
 *
 * When generated by the host, it is sent to erase & write a page (528 bytes) worth of data into the Atmel dataflash.
...
typedef struct PACKED_ATTRIBUTE {
    ...
    uint8_t data[FLASH_PAGE_SIZE];          /// Array of 528 bytes of flash data. Index 0 maps to the 1st byte in the page.
} PacketFlashPayload;
