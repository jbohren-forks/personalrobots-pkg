...
double laser_encoder_offset = 1.57; //1.5252; //1.4912; //85.0 * PI/180.0;
// tilt of camera about x-axis (of laser frame)
double laser_tilt_offset = 0; //-0.0786; //1.4912; //-5.0 * PI/180.0;
...
vector<double> intrinsics(double xp, double yp)
{
  ...
  // measured by Morgan:
  double fx = 533.43634, fy = 532.25193;
  double x0 = 329.769073, y0 = 265.015808; //335.106, y0 = 260.255;
...
void generate_cloud() 
{
  ...
  cv->clear_cloud();
    ...
    wx = P[0];
    wy =  cos(wrot) * P[1] + sin(wrot) * P[2];
    wz = -sin(wrot) * P[1] + cos(wrot) * P[2];
