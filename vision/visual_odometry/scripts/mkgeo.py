...
#
# geometry statistics
# parses a bag into two parts, runs geometry check on pairs from each,
...
#        xs -= 4.5 * 1e-3
        ...
        xk = [ x for j,x in enumerate(vo_x[0]) if j in vo.log_keyframes ]
        yk = [ y for j,y in enumerate(vo_y[0]) if j in vo.log_keyframes ]
...
print "There are", len(frames1), "frames in the first set"
print "There are", len(frames2), "frames in the second set"
