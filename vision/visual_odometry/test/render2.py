...
class ray_camera:
    ...
    def genray(self, sx, sy, side):
        scr = self.pcenter[side] + self.right[side] * (sx + self.xcenter) + self.up[side] * (sy + self.ycenter)
        return ray(self.center[side], (scr - self.center[side]).normalize())
...
def render_stereo_scene(imL, imR, imD, cam, scene):
    im = imL
