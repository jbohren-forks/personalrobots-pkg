...
def homo_to_cvpoint(p):
    p = p / p[-1,0]
    return cv.cvPoint(int(round(p[0,0])), int(round(p[1,0])))
...
if __name__ == "__main__":
    ...
    if 'test_hdr' == test:
        hg.cvNamedWindow("left_eye",  1)
        hg.cvNamedWindow("right_eye", 1)

        hg.cvNamedWindow("left_eye_low",  1)
        hg.cvNamedWindow("right_eye_low", 1)

        cam = VidereHDR(0)
        base_exposure = 100

        for ll, lr, l, r in cam:
            hg.cvShowImage("left_eye_low", ll)
            hg.cvShowImage("right_eye_low", lr)
            hg.cvShowImage("left_eye", l)
            hg.cvShowImage("right_eye", r)
            key = hg.cvWaitKey(10)
