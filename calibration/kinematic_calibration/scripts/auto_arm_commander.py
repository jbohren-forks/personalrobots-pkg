...
def mech_state_callback(data, interval_publisher):
	...
	if (ready_to_capture) :
		if (len(joint_state_hist) == N) :
			state_maxes = [ max( [x[1][k] for x in joint_state_hist] ) for k in range(0,len(traj_joint_names)) ]
			state_mins  = [ min( [x[1][k] for x in joint_state_hist] ) for k in range(0,len(traj_joint_names)) ]
			state_range = [state_maxes[k]-state_mins[k]             for k in range(0,len(traj_joint_names)) ]
			...
			if ( max(state_range) > .00000001 ) :
				...
				capture_publisher.publish(Capture(roslib.msg.Header(None, joint_state_hist[0][0], None)))
				ready_to_capture = False
				#done_capturing = True

def cal_sample_callback(data, args) :
	global done_capturing ;
	done_capturing = True
	print "CalSampleCallback!"
...
if __name__ == '__main__':
	...
	interval_publisher = rospy.Publisher('stationary_interval', Interval)
        capture_publisher = rospy.Publisher('capture', Capture) ;
#	interval_publisher = [ ]
	sub = rospy.Subscriber("mechanism_state", MechanismState, mech_state_callback, interval_publisher)
        sub = rospy.Subscriber("cal_sample", CameraCalSample, cal_sample_callback, [ ])
