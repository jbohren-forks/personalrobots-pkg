...
void FormMain::OnResize( wxSizeEvent& event )
{
    ...
    // Using m_pPropGridMan, we can scan all pages automatically.
    p = m_pPropGridMan->GetPropertyByName( wxT("Width") );
    if ( p && !p->IsValueUnspecified() )
        m_pPropGridMan->SetPropertyValue( p, w );

    p = m_pPropGridMan->GetPropertyByName( wxT("Height") );
    if ( p && !p->IsValueUnspecified() )
        m_pPropGridMan->SetPropertyValue( p, h );
...
void FormMain::OnPropertyGridChanging( wxPropertyGridEvent& event )
{
    ...
    //wxLogDebug( wxT("About to change value of '%s' to '%s'"), p->GetName().c_str(), event.GetValue().GetString().c_str() );
...
void FormMain::OnPropertyGridChange( wxPropertyGridEvent& event )
{
    ...
    //wxLogDebug( wxT("Changed value of '%s' to '%s'"), name.c_str(), event.GetPropertyValueAsString().c_str() );
...
{
    wxPropertyGridPage* page = m_pPropGridMan->GetCurrentPage();

    // Remove auto-centering
    m_pPropGridMan->SetWindowStyle( m_pPropGridMan->GetWindowStyle() & ~wxPG_SPLITTER_AUTO_CENTER);

    // Grow manager size just prior fit - otherwise
    // column information may be lost.
    wxSize oldGridSize = m_pPropGridMan->GetGrid()->GetClientSize();
...
#if SIMPLE_DEMO || !TESTING_WXPROPERTYGRIDADV
...
#endif // SIMPLE_DEMO || !TESTING_WXPROPERTYGRIDADV
...
void FormMain::CreateGrid( int style, int extraStyle )
{
    ...
    */
    ...
    m_topSizer->Add( m_pPropGridMan, 1, wxEXPAND );

#if !SIMPLE_DEMO
    // Additional control (for keyboard navigation testing)
    m_topSizer->Add( new wxButton(m_panel,1234,wxT("Button for Tab Traversal Testing")), 0, wxEXPAND|wxFIXED_MINSIZE );
#endif
