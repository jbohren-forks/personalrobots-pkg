...
int mushroom_read_database( const char* filename, CvMat** data, CvMat** missing, CvMat** responses )
{
	...
	for(i=0, ptr = buf; *ptr != '\0';++i, ptr++ ){
		var_count += *ptr == ',';
	}
	assert( ptr - buf == (var_count+1)*2+1 ); //I put the final "+1" to account for Window's lf
...
int main( int argc, char** argv )
{
	...
	printf("SAVING TREE\n");
	dtree->save("tree2.xml","Gary"); 
	CvDTree dtree1;
	printf("CLEAR TREE, RE-LOAD IT AND TEST\n");
    dtree = mushroom_read_dtree(&dtree1, data, missing, responses,
        10 // poisonous mushrooms will have 10x higher weight in the decision tree
        );
	printf("DONE\n");
