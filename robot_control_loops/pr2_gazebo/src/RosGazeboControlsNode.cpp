...
void
RosGazeboControlsNode::cmd_rightarmconfigReceived()
{
  this->lock.lock();
  newRightArmPos = true;
  /*
  ...
  */
  ...
  if(!(useControllerArray||useArmController)){
...
void
RosGazeboControlsNode::cmd_leftposeReceived(){
  this->lock.lock();
  newLeftPose = true;
  std::cout<<"Left pose received:"<<cmd_leftpose.x<<" "<<cmd_leftpose.y<<" "<<cmd_leftpose.z<<" "<<cmd_leftpose.roll<<" "<<cmd_leftpose.pitch<<" "<<cmd_leftpose.yaw<<std::endl;

  this->lock.unlock();
}

void
RosGazeboControlsNode::cmd_rightposeReceived(){
  this->lock.lock();
  newRightPose = true;
  this->lock.unlock();
}
...
void
RosGazeboControlsNode::cmd_leftarmconfigReceived()
{
  this->lock.lock();
  newLeftArmPos = true;
...
void RosGazeboControlsNode::cmd_leftarmcartesianReceived() {
  this->lock.lock();
...
void RosGazeboControlsNode::cmd_rightarmcartesianReceived() {
  this->lock.lock();
...
void
RosGazeboControlsNode::cmdvelReceived()
{
  this->lock.lock();
  double dt;
...
RosGazeboControlsNode::RosGazeboControlsNode(int argc, char** argv, const char* fname,
         PR2::PR2Robot          *myPR2,
...
int
RosGazeboControlsNode::AdvertiseSubscribeMessages()
{
  ...
  subscribe("cmd_vel", velMsg, &RosGazeboControlsNode::cmdvelReceived);
  subscribe("cmd_leftarmconfig", leftarm, &RosGazeboControlsNode::cmd_leftarmconfigReceived);
  subscribe("cmd_rightarmconfig", rightarm, &RosGazeboControlsNode::cmd_rightarmconfigReceived);
  subscribe("cmd_leftarm_cartesian", cmd_leftarmcartesian, &RosGazeboControlsNode::cmd_leftarmcartesianReceived);
  subscribe("cmd_rightarm_cartesian", cmd_rightarmcartesian, &RosGazeboControlsNode::cmd_rightarmcartesianReceived);
  subscribe("cmd_leftpose",cmd_leftpose, &RosGazeboControlsNode::cmd_leftposeReceived);
  subscribe("cmd_rightpose",cmd_rightpose, &RosGazeboControlsNode::cmd_rightposeReceived);

  return(0);
}

RosGazeboControlsNode::~RosGazeboControlsNode()
{
}

double
RosGazeboControlsNode::GaussianKernel(double mu,double sigma)
{
...
void
RosGazeboControlsNode::UpdateRightArm(){
  if(useArmController){ //Send commands via ArmContrller
    double position[controller::ArmController::ARM_MAX_JOINTS] = {this->rightarm.turretAngle,this->rightarm.shoulderLiftAngle,this->rightarm.upperarmRollAngle,this->rightarm.elbowAngle, this->rightarm.forearmRollAngle,this->rightarm.wristPitchAngle,this->rightarm.wristRollAngle};
   myArm->setArmJointPos(position, NULL); 
  }
  else{
...
void
RosGazeboControlsNode::UpdateLeftArm(){
  
  if(useArmController){ 
  double position[controller::ArmController::ARM_MAX_JOINTS] = {this->leftarm.turretAngle,this->leftarm.shoulderLiftAngle,this->leftarm.upperarmRollAngle,this->leftarm.elbowAngle, this->leftarm.forearmRollAngle,this->leftarm.wristPitchAngle,this->leftarm.wristRollAngle};
  myArm->setArmJointPos(position,NULL);
...
RosGazeboControlsNode::Update()
{
  ...
  if(newLeftPose) {
  myArm->setHandCartesianPos(cmd_leftpose.x,cmd_leftpose.y,cmd_leftpose.z,cmd_leftpose.roll,cmd_leftpose.pitch, cmd_leftpose.yaw);
  newLeftPose = false; 
}
