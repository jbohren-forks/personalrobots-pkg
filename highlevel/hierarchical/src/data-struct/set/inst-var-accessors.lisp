...
(defstruct (inst-var-accessors (:conc-name acc-) (:constructor create-inst-var-accessors))
  creator
  readers
  writers
...
(defun make-inst-var-accessors (&key creator readers writers invalid-inst-detector var-num var-names)
  "make-inst-var-accessors &key CREATOR READERS WRITERS (INVALID-INST-DETECTOR (constantly nil)) VAR-NUM VAR-NAMES

The most general way to create inst-var-accessors (see help for 'inst-vars).  
CREATOR is a function of zero arguments that returns a (fresh) instantiation with all variables uninstantiated.
READERS is a sequence whose Ith element is a function of one argument, that takes in an instantiation and returns the value of the Ith variable.
WRITERS is a sequence whose Ith element is a function of two arguments, Y and INST, which sets the value of variable I in INST to be Y.
INVALID-INST-DETECTOR is a function of one argument INST, that checks if INST is a well-formed instantiation.  If so, it returns nil.  If not, it returns a non-nil object that denotes the reason (typically in a human-readable way).  Defaults to constantly nil.
VAR-NUM is a function of one argument, a variable name, that returns the number of this variable.  May be omitted.
VAR-NAMES is a [numbered-set] containing the variable names.  Defaults to just 0,1,..."
  (create-inst-var-accessors
   :creator (check-not-null creator)
   :readers (coerce (check-not-null readers) 'vector)
   :writers (coerce (check-not-null writers) 'vector)
   :invalid-inst-detector (or invalid-inst-detector (constantly nil))
   :var-num (or var-num #'(lambda (name) (item-number name var-names)))
   :var-names (or var-names (length readers))))
...
(defgeneric set-var-val (a x i val)
  ...
  (:method ((a inst-var-accessors) x i val)
	   (funcall (aref (acc-writers a) i) val x)))
...
(defstruct (list-accessors (:conc-name la-) (:constructor create-list-accessors))
  names
  prefix)

(defun make-list-accessors (n &key prefix)
  "make-list-accessors N &key PREFIX.  N is either a list of variable names, or a fixnum, which represents the list '(0 1 ... n-1).  Instantiations are then stored as lists of values."
  (create-list-accessors 
   :names
   (etypecase n
     (fixnum (below n))
     (list n))
...
(defmethod create-inst ((a list-accessors))
  (with-struct (la- names prefix) a
    (let ((l (make-list (length names) :initial-element 'uninstantiated)))
      (if prefix
	  (cons prefix l)
	l))))
	

(defmethod get-var-val ((a list-accessors) x i)
  (if (listp x)
      (nth (+ i (indicator (la-prefix a))) x)
    'illegal-instantiation))

(defmethod set-var-val ((a list-accessors) x i val)
  (setf (nth (+ i (indicator (la-prefix a))) x) val))

(defmethod get-var-num ((a list-accessors) name)
  (check-not-null (position name (la-names a) :test #'equalp)
		  "var ~a not found in ~a" name (la-names a)))

(defmethod var-names ((a list-accessors))
  (la-names a))

(defmethod detect-invalid-inst ((a list-accessors) i)
  (with-struct (la- names prefix) a
    (if (or (null prefix) (eql prefix (car i)))
	(loop
	    with n = (length names)
	    with k = (if prefix (cdr i) i)
	    for j below n
	  ...
	  (coerce 
	   (append
	    ,@(loop
		  for f in field-descs
		  if (symbolp f) 
		  collect `(list #'(lambda (,val ,x) (setf (,(make-acc f) ,x) ,val)))
		  else 
		  collect
		  `(mapset
		    'list
		    (lambda (,i)
		      #'(lambda (,val ,x) (setf (aref (,(make-acc (first f)) ,x) ,i) ,val)))
		    ,(third f))))
	   'vector)
